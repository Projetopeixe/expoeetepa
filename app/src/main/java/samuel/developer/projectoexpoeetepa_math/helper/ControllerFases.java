package samuel.developer.projectoexpoeetepa_math.helper;

import android.content.SharedPreferences;

public class ControllerFases {

    int fase1 = 1;
    int fase2 = 0;
    int fase3 = 0;
    int fase4 = 0;
    int fase5 = 0;
    int fase6 = 0;
    int fase7 = 0;
    int fase8 = 0;
    int fase9 = 0;
    int fase10 = 0;


    public int getFase1() {
        return fase1;
    }

    public void setFase1(int fase1) {
        this.fase1 = fase1;
    }

    public int getFase2() {
        return fase2;
    }

    public void setFase2(int fase2) {
        this.fase2 = fase2;
    }

    public int getFase3() {
        return fase3;
    }

    public void setFase3(int fase3) {
        this.fase3 = fase3;
    }

    public int getFase4() {
        return fase4;
    }

    public void setFase4(int fase4) {
        this.fase4 = fase4;
    }

    public int getFase5() {
        return fase5;
    }

    public void setFase5(int fase5) {
        this.fase5 = fase5;
    }

    public int getFase6() {
        return fase6;
    }

    public void setFase6(int fase6) {
        this.fase6 = fase6;
    }

    public int getFase7() {
        return fase7;
    }

    public void setFase7(int fase7) {
        this.fase7 = fase7;
    }

    public int getFase8() {
        return fase8;
    }

    public void setFase8(int fase8) {
        this.fase8 = fase8;
    }

    public int getFase9() {
        return fase9;
    }

    public void setFase9(int fase9) {
        this.fase9 = fase9;
    }

    public int getFase10() {
        return fase10;
    }

    public void setFase10(int fase10) {
        this.fase10 = fase10;
    }
}
